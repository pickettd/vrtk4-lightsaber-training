...
public class RotateMultiTowardsAndFire : MonoBehaviour {
    ...
    [Tooltip("Optional second target. If provided then the drone will shoot at the position in between the two targets")]
    ...
    void Rotate()
    {
        ...
        Vector3 shootTargetPosition = target.transform.position;
        if (target2 != null && target2.transform)
        {
            shootTargetPosition = Vector3.Lerp(target.transform.position, target2.transform.position, 0.5f);
        }
        Vector3 targetDir = shootTargetPosition - child.transform.position;
